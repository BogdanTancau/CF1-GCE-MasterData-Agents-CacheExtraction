...
def RetrieveFilesForCompany(company):
    try:
        ...
        localFilePath = "{0}{1}/snow_export/".format(localPath, company.Ident)
        
        bucketFilePath = "audlExtraction/ClientData{0}/snow_export/".format(company.Ident)
        ...
        with pysftp.Connection(host=sftpHost, port=sftpPort, username=sftpUserName, private_key=sftpKey, cnopts=cnopts) as sftpClient:
            ...
            for filename in files:
                ...
                if fnmatch.fnmatch(filename, "*.csv"):
                    ...
                    with open(copy_file_destination, "r", encoding="utf-16") as reader:
                        reader.close()
                        # Open copy download file from local server
                        # Convert File to UTF16 and relocate to proper Location
                        # Rename file removing utf8, snow_atosglobal{0}_ and spaces
                        newFileName = filename.replace("utf8", "UTF16")
                        newFileName = newFileName.replace("snow_atosglobal{0}_".format("" if company.SNOWTarget == "prod" else company.SNOWTarget), "")
                        newFileName = newFileName.replace(" ", "")
                        newFileName = newFileName.replace("export", "")
                        # copy file with encoding utf8 to utf 16 and into directory
                        if not os.path.exists(importPath):
                            os.makedirs(importPath)
                        CopyContents(copy_file_destination, importPath + newFileName)
                    logging.info(f"Filed converted successfully: {importPath + newFileName}")
                    print(f"Filed converted successfully: {importPath + newFileName}")
                    
                    print(f"Second upload to bucket: {newFileName}")
                    upload_to_bucket("gce-master-data", importPath + newFileName, "snowCacheFile/" + newFileName)
